...
st.markdown("基于多维数据加权与 60 天趋势预测的 TikTok Shop 深度选品工具")
...
def add_log(title, request_data, response_data):
    ...
    st.session_state.app_logs.insert(0, log_content)
...
def fetch_products(region, l3_id, rank_type, limit):
    ...
    if rank_type == 1:
        ...
        target_date = last_day_prev_month.replace(day=1).strftime("%Y-%m-%d")
    ...
    try:
        ...
        add_log(f"📊 大盘拉取 ({target_date})", params, f"成功获取 {len(res.get('data', []))} 条数据" if res.get("code")==0 else res)
        return res.get("data", [])
...
def fetch_potential_index(product_id, rank_type):
    ...
    try:
        ...
        # 动态趋势算法
        if rank_type == 1: # 日榜：近3天 vs 前7天
            recent_avg = sum(daily_sales[:3]) / 3
            history_avg = sum(daily_sales[3:10]) / 7
        elif rank_type == 2: # 周榜：近7天 vs 前14天
            recent_avg = sum(daily_sales[:7]) / 7
            history_avg = sum(daily_sales[7:21]) / 14 if len(daily_sales) >= 21 else 1
        else: # 月榜：近15天 vs 前30天
            recent_avg = sum(daily_sales[:15]) / 15
            history_avg = sum(daily_sales[15:45]) / 30 if len(daily_sales) >= 45 else 1
            
        history_avg = max(history_avg, 0.1)
        ratio = recent_avg / history_avg
        
        add_log(f"📈 趋势测算 ({product_id})", f"周期:{rank_type}", f"比值: {ratio:.2f}")

        if ratio > 1.25: return f"🔥 显著上升 (近况 {int(recent_avg)}件/天)", recent_avg
        elif ratio < 0.8: return f"📉 正在下降 (近况 {int(recent_avg)}件/天)", recent_avg
        else: return f"➡️ 趋势平稳 (近况 {int(recent_avg)}件/天)", recent_avg
...
with st.sidebar:
    ...
    # 🌟 修复2：不在侧边栏立即渲染日志，而是准备一个“空盒子”，等主程序跑完再填入数据
...
if st.button("🚀 开始 AI 智能选品", type="primary", use_container_width=True):
    ...
    if not raw_products:
        st.warning("未能拉取到数据，请查看左侧【系统运行日志】排查具体原因。")
    else:
        for idx, p in enumerate(raw_products):
            ...
            with st.container(border=True):
                ...
                with col1:
                    ...
                    if "上升" in p_trend:
                        st.success(f"🔮 **动态潜力预测**: {p_trend}")
                    elif "下降" in p_trend:
                        st.error(f"🔮 **动态潜力预测**: {p_trend}")
                    else:
                        st.info(f"🔮 **动态潜力预测**: {p_trend}")
                ...
                with col2:
                    ...
                    st.write(f"👥 关联达人数: **{p.get('total_lfl_cnt') or 0}**")
                    st.write(f"🎥 关联视频数: **{p.get('total_video_cnt') or 0}**")
                ...
                with btn_col1:
                    st.link_button("🔗 在 EchoTik 查看详情(含图)", f"https://echotik.live/products/{p_id}", use_container_width=True)
                with btn_col2:
...
# ==========================================
# 7. 日志最终渲染 (等待主程序所有动作完毕后再填充侧边栏盒子)
# ==========================================
with log_container.container():
    with st.expander("点击展开详情", expanded=False):
        if st.session_state.app_logs:
            for log in st.session_state.app_logs[:15]: 
                st.text(log)
                st.markdown("---")
        else:
            st.caption("暂无运行记录...")
